...
Subscriptions::Subscriptions(): Money(0,0)
{
    name_subscriptions="-";
}
...
std::string Subscriptions::getPrice()
{
    ...
    if(key==1)
    {
        return price="+"+std::to_string(getRub())+"."+std::to_string(getKopeck());
    }else if(key==2) return price="-"+std::to_string(getRub())+"."+std::to_string(getKopeck());
}
...
int Subscriptions::balanceOperation(Person *P1)
{
    if(frequency_sub==1) time.nextDay(1);
        ...
        if(key==1)
        {
            P1->OperationSumMoney(getRub(),getKopeck());
            return 1;
        }else if(key==2)
        {
            if(P1->OperationComparisonMoney(getRub(),getKopeck())==1)
            {
                P1->OperationMinusMoney(getRub(), getKopeck());
                return 1;
            }
            else if(P1->OperationComparisonMoney(getRub(),getKopeck())==2) return 0;
        }else P1->OperationMinusMoney(getRub(), getKopeck());
}
